            ...
            <div className={`flex space-x-3 sm:space-x-4 md:space-x-5 lg:space-x-6 ${shouldAnimate ? 'desktop-animate' : testimonials.length < 3 ? 'justify-center' : 'justify-start'}`}>
              {displayTestimonials.map((testimonial, index) => (
                <div
                  ...
                  className="shrink-0 w-[85vw] xs:w-[340px] sm:w-[360px] md:w-[380px] lg:w-[400px] xl:w-[420px] bg-white rounded-lg sm:rounded-xl shadow-md hover:shadow-xl p-5 sm:p-6 md:p-7 lg:p-8 border border-gray-200 hover:border-orange-500 transition-all duration-300 mb-4"
                >
                  {/* Quote Icon */}
                  <div className="mb-4 sm:mb-5">
                    <FaQuoteLeft className="text-xl sm:text-2xl md:text-3xl text-orange-500 opacity-50" />
                  </div>


                  {/* Testimonial Text */}
                  <p className="text-gray-700 leading-relaxed mb-4 sm:mb-5 text-xs sm:text-sm md:text-base line-clamp-4 sm:line-clamp-5">
                    "{testimonial.message_text}"
                  </p>


                  {/* Rating Stars - Fixed 5 stars */}
                  <div className="flex space-x-1 mb-4 sm:mb-5">
                    {[...Array(5)].map((_, i) => (
                  ...
                  {/* User Info */}
                  <div className="flex items-center space-x-3 sm:space-x-4 pt-4 sm:pt-5 border-t border-gray-200">
                    <img
                      src={testimonial.image_url || getDummyImage(testimonial.author_name)}
                      alt={testimonial.author_name}
                      className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full object-cover border-2 sm:border-[3px] border-orange-200"
                    />
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm sm:text-base md:text-lg">{testimonial.author_name}</h4>
                      <p className="text-xs sm:text-sm text-orange-600">Community Member</p>
                    </div>
                  </div>
                </div>
              ))}
