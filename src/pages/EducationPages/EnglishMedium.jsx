  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/education/en`);
        if (!res.ok) {
        ...
        // Map API response - KEEP HTML tags for proper formatting
        const englishMediumSchools = (response.data || [])
          .map(item => ({
            id: item.id,
            name: item.title,
            description: item.description, // HTML tags preserved
            image: item.image_url || item.image,
          }));

        setSchools(englishMediumSchools);
      } catch (err) {
        console.error(err);
    ...
    return (
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-linear-to-t from-cream to-white text-navy">
          <p className="text-lg font-semibold">Loading English Medium Schools...</p>
        </div>
      </>
    );
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-linear-to-t from-cream to-white text-navy">
        ...
        <header className="p-4 sm:ml-24 -mb-2 -mt-2">
          <h1 className="sm:text-4xl text-2xl font-bold">English Medium Schools</h1>
        </header>
        ...
        <main className="container mx-auto p-6 pt-12 pb-10">
          {schools.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-xl text-gray-500">No English medium schools found.</p>
            </div>
          ) : (
