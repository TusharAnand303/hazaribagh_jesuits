  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        ...
        // medium = higins (as you used in API)
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/education/higins`);
        if (!res.ok) {
        ...
        const higherInstituteSchools = (response.data || []).map(item => ({
          id: item.id,
          name: item.title,
        ...
        setSchools(higherInstituteSchools);
      } catch (err) {
        console.error(err);
        setError('Unable to load higher institutions. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-linear-to-t from-cream to-white text-navy">
          <p className="text-lg font-semibold">Loading Higher Institutions...</p>
        </div>
      </>
    );
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-linear-to-t from-cream to-white text-navy">
        ...
        <header className="p-4 sm:ml-24 -mb-2 -mt-2">
          <h1 className="sm:text-4xl text-2xl font-bold">Higher Institutions</h1>
        </header>
        ...
        <main className="container mx-auto p-6 pt-12 pb-10">
          {schools.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-xl text-gray-500">No higher institutions found.</p>
            </div>
          ) : (
...
export default HigherInstitute;