...
const SocialCenters = () => {
    ...
    const fetchSocialCenters = async () => {
      try {
        ...
        if (response.status && response.data) {
          ...
          document.title = `${response.data.title} - Social Centers`;
        } else {
          setError('Social center not found');
        }
      } catch (err) {
        console.error(err);
        setError(
          err.message ||
            'Unable to load Social Centers details. Please try again later.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const breadcrumbItems = [
    { label: 'Home', path: '/' },
    { label: content?.title || 'Details', path: `/socialcenters/${id}` },
  ];
    ...
    return (
      <div className="min-h-screen bg-cream">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-gray-600 text-lg">Loading social center details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-cream">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <h3 className="text-xl font-bold text-navy">Unable to Load Social Center</h3>
          <p className="text-gray-600">
            {error === 'Request timeout'
              ? 'The request is taking longer than expected.'
              : error || 'Social center details could not be found.'}
          </p>
          <Link to="/socialcenters">
            <button className="px-6 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg transition-colors">
              Back to Social Centers
            </button>
          </Link>
        </div>
      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-gray-500 font-medium text-sm">Social Center Name</span>
